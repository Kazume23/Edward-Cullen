...
<body>
  ...
  <script>
    ...
    // PHP wrzuca info czy zalogowany
    const AUTH_USER = <?php echo json_encode($authUser); ?>;
    ...
    // jeśli użytkownik się zalogował pierwszy raz, a miał dane offline
    ...
    async function flushServerSync(reason) {
      ...
      try {
        await apiPutState(reason || "flush");
        syncBackoffMs = 2000;
      } catch (e) {
        syncPending = true;
...
const baseId = "baseStyles"; // TO NIGDY NIE MOZE BYC WYLACZONE
const themeIds = ["themeStyles", "themelight", "themepink"]; // same nadpiski
...
function applyTheme(themeId) {
  ...
  themeIds.forEach(id => {
    ...
    // media: "not all" = zaladowane, ale nie stosowane, zero migania
    link.disabled = false;
...
if (themeBtn) {
  themeBtn.addEventListener("click", () => {
    ...
    function saveState(opts = {}) {
      ...
      if (sync) queueServerSync("saveState");
    }
  ...
  (() => {
    ...
    if (overlay.getAttribute("aria-hidden") === "false") overlay.classList.add("isOpen");
